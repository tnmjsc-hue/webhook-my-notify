...
import { PLAN_NAMES, type BillingCycle, type PlanName } from '@/lib/plans'
...
export async function POST(request: Request) {
  ...
  const { error } = await serviceRole
    .from('profiles')
    .update({ plan: newPlan, billing_cycle: billingCycle })
    .eq('id', user.id)
  ...
  return NextResponse.json({ ok: true, plan: newPlan, billingCycle })
}
